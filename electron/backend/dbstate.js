...
//
// FINANCE_LAB_DB_PATH (the test suite) bypasses the pointer file entirely and
...
function dataDir() {
  const d = process.env.FINANCE_LAB_DATA_DIR;
  if (d) {
    fs.mkdirSync(d, { recursive: true });
    return d;
  }
  // Non-Electron fallback (dev/tests without the env var): a .data dir under
  // the cwd. The packaged app always sets FINANCE_LAB_DATA_DIR (main.js).
...
function createDbState() {
  ...
  const dbFilePath = () =>
    process.env.FINANCE_LAB_DB_PATH || path.join(dataDir(), 'finance.db');

  function loadInitialState() {
    const explicit = process.env.FINANCE_LAB_DB_PATH;
    if (explicit) {
  ...
  function savePointer() {
    if (process.env.FINANCE_LAB_DB_PATH) return;
