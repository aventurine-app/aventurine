...
// via OLIV_DB_PATH (which also suppresses pointer-file writes), and
...
function makeClient(t) {
  ...
  const prev = process.env.OLIV_DB_PATH;
  process.env.OLIV_DB_PATH = dbPath;
  const conn = createConn();
  conn.init();
  t.after(() => {
    conn.closeAll();
    if (prev === undefined) delete process.env.OLIV_DB_PATH;
    else process.env.OLIV_DB_PATH = prev;
